...
export default function TestBonzaiPage() {
  ...
  const { data, isLoading, error } = useQuery<ApiResponse>({
    queryKey: ['bonzai-test'],
    queryFn: async () => {
      const response = await fetch('/api/bonzai-test');
      if (!response.ok) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['bonzai-test'] });
      reset();
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, formData }: { id: string; formData: UpdateFormData }) => {
      const response = await fetch(`/api/bonzai-test/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
