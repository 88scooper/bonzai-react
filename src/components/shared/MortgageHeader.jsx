...
/**
 * MortgageHeader Component
 * Prominent Bonzai Green banner header for mortgage detail views
 * Features property name, lender, and edit button
 * Mercury + Bonzai Green aesthetic
 */
export default function MortgageHeader({ 
  mortgage,
  property,
  onEdit,
  className = ""
}) {
  const propertyName = property?.nickname || property?.name || mortgage?.propertyName || 'Property';
  const propertyAddress = property?.address || mortgage?.property?.address || '';
  const lenderName = mortgage?.lenderName || mortgage?.lender || 'Mortgage';
  const mortgageAmount = mortgage?.originalAmount || mortgage?.mortgage?.originalAmount;
      ...
      <div className="relative px-6 py-6 md:px-8 md:py-8">
        <div className="flex items-start justify-between gap-4 md:gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {propertyName && (
              <p className="text-base md:text-lg text-white/90 dark:text-white/80 mb-1">
                {propertyName}
              </p>
            )}
            {propertyAddress && (
              <p className="text-sm md:text-base text-white/80 dark:text-white/70 mb-2">
                {propertyAddress}
              </p>
            )}
            {mortgageAmount && (
              <p className="text-sm md:text-base font-semibold text-white/90">
                {new Intl.NumberFormat('en-CA', { style: 'currency', currency: 'CAD' }).format(mortgageAmount)}
              </p>
            )}
          </div>
